...
def draw_text(screen_surf, text, y, x=None):
    if x == None:
        x = (WIDTH - sum([char_width(c) for c in text])) // 2

    for char in text:
        screen_surf.blit("font0"+str(ord(char)), (x, y))
        x += char_width(char)
...
class Player(GravityActor):
    def __init__(self, game_instance):
        ...
        self.lives = 2
        self.score = 0
        self.app_input = None # Will be set by App
    ...
    def update(self):
        ...
        if self.hurt_timer > 100:
            ...
            # Use the app's input handler via game.app
            if self.game.app.input.space_pressed() and self.fire_timer <= 0 and len(self.game.orbs) < 5:
...
class Game:
    def __init__(self, app, player=None):
        ...
        self.player = player
        if self.player:
            # Player needs back reference to game to access other entities
            # We must set this manually since Player is created before Game in some contexts
            # or passed in.
            # However, simpler to just ensure player is instantiated with 'self' if we created it inside.
            # Since we pass player in, we patch the game reference.
    ...
    def play_sound(self, name, count=1):
        # Only play sound if this game instance actually has a player (active game)
        # or if it's the specific case of menu music logic handled elsewhere
        if self.player:
            try:
    ...
    def draw_status(self):
        ...
        draw_text(screen, "LEVEL " + str(self.level + 1), 451)
...
class MenuScreen:
    def __init__(self, app):
        self.app = app
        # Create a game for background visual, no player attached
        self.bg_game = Game(app, player=None)

    def update(self):
        if self.app.input.space_pressed():
    ...
    def draw(self):
        ...
        # Use bg_game.timer for animation sync
...
class PlayScreen:
    def __init__(self, app):
        self.app = app
        # Create a dummy player first to pass to Game
        p = Player(None) # Game will be set in Game init
